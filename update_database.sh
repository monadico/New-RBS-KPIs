...
if [ ! -f "/app/betting_transactions.db" ]; then
    log_message "Database file not found, creating empty database file first..."
    touch /app/betting_transactions.db
    chmod 666 /app/betting_transactions.db
    log_message "Empty database file created, running initial database creation..."
    python3 betting_database.py --db-path /app/betting_transactions.db --start-block 0
    if [ $? -eq 0 ]; then
...
log_message "Database file exists: $(ls -la /app/betting_transactions.db 2>/dev/null || echo 'No database file')"
python3 betting_database.py --db-path /app/betting_transactions.db --incremental
